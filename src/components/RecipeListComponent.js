      ...
      >
        ...
        <View style={styles.contentButton}>
          ...
          <View style={styles.contentEdit}>
            <TouchableOpacity
              onPress={() => this.props.navigation.navigate("EditRecipeScreen", {
                id: item.id,
                name: item.name,
                image: item.image,
                ingredienrts: item.ingredienrts,
              ...
              <Image source={imgEdit} style={{ width: 15, height: 15, tintColor: 'gray' }} />
            </TouchableOpacity>

            <TouchableOpacity
              onPress={() => this.alertDelete(item.id, item.name)}
              style={styles.button}>
              <Image source={imgDelete} style={{ width: 18, height: 18, tintColor: 'red' }} />
            </TouchableOpacity>
          </View>
        </View>
      </LinearGradient>
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        ...
        >
          <TouchableOpacity
            onPress={() => this.props.navigation.navigate("AddRecipeScreen", {
              image: this.props.navigation.state.params.image,
              name: this.props.navigation.state.params.name,
              refresh: this.refreshData
            })}
            style={{ paddingHorizontal: 10, alignItems: 'center' }}>
            <Image source={imgAdd} style={{ width: 25, height: 25, tintColor: 'white' }} />
          </TouchableOpacity>
        </LinearGradient>
        ...
        <View style={styles.flatList}>
          {data.length <= 0 ?
            <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
              <Image
                source={imgRecipe}
                style={{ width: 150, height: 150, marginTop: 10, tintColor: 'gray', marginBottom: 20 }}
              />
              <Text style={{ fontSize: 20, color: 'gray', margin: 10 }}>List of empty recipes</Text>
            </View> :
            <FlatList
